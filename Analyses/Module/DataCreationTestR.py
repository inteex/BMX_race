...
class DataCreationTestR:
    ...
    def Creation_Test_Power(self,Path,Labels,Name):
        ...
        for element in fls_sortie:
            ...
            X = pd.read_csv('Données\\Traitements\\' + element,sep=",",encoding='Latin',engine='python')[Variable]
            N.append(len(X))
        ...
        Output.index = ID
        Output.to_csv('R\\Nouveaute\\' + Name,sep=",",header=True,index=True)
    ...
    def Creation_Test_Feature(self,Path,Labels,Variable,Name):
        ...
        for element in fls_sortie:
            ...
            X = pd.read_csv('Données\\Traitements\\' + element,sep=",",encoding='Latin',engine='python')["Puissance"]
            if (len(X) >= N):
                ID.append(element.split('_')[1] + "_" + element.split('_')[2])
                Output.append(list(X)[0:N])
            else :
